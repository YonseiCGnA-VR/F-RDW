...
public static class Utilities
{
    ...
    {
        Vector3 prevTmp = new Vector3(prevDir.x, 0, prevDir.y);
        Vector3 currTmp = new Vector3(currDir.x, 0, currDir.y);
        return GetSignedAngle(prevTmp, currTmp);
    }
    ...
    public static float CalculateAngle(Vector3 baseLine, Vector3 Vector)
    {
        return Quaternion.FromToRotation(baseLine, Vector).eulerAngles.y;
    }

    public static float CalculateAngle(Vector2 baseLine, Vector2 Vector)
    {
        Vector3 baseTmp = new Vector3(baseLine.x, 0, baseLine.y);
        Vector3 VectorTmp = new Vector3(Vector.x, 0, Vector.y);
        return CalculateAngle(baseTmp, VectorTmp);
    }
    ...
    public static Vector3 RotateVector(Vector3 fromOrientation, float thetaInDegrees)
    {
        Vector3 ret = Vector2.zero;
        float cos = Mathf.Cos(-thetaInDegrees * Mathf.Deg2Rad);
        float sin = Mathf.Sin(-thetaInDegrees * Mathf.Deg2Rad);
        ret.x = fromOrientation.x * cos - fromOrientation.z * sin;
        ret.y = 0;
        ret.z = fromOrientation.x * sin + fromOrientation.z * cos;
        return ret;
    }
    ...
    public static bool isOutOfBoundaries(Vector2 pos, List<Vector2> trackingBoundaries, List<List<Vector2>> obstaclePolygons)
    {
        bool result = false;
        for (int i = 0; i < trackingBoundaries.Count; i++)
        {
            var p = trackingBoundaries[i];
            var q = trackingBoundaries[(i + 1) % trackingBoundaries.Count];
            if (!(Vector2.Dot(q - p, pos - p) > 0 && Vector2.Dot(p - q, pos - q) > 0))
            {
                result = true;
            }
    ...
    public static bool isOutOfBoundaries(Vector2 pos, List<Vector2> trackingBoundaries, List<List<Vector2>> obstaclePolygons, ref Vector2 boundaryStartPoint, ref Vector2 boundaryEndPoint)
    {
        bool result = false;
        for (int i = 0; i < trackingBoundaries.Count; i++)
        {
        ...
        if (!result)
        {
            boundaryStartPoint = new Vector2(0, 0);
            boundaryEndPoint = new Vector2(0, 0);
        }
