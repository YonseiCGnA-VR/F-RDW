...
public class GlobalConfiguration : MonoBehaviour
{
    ...
    public enum TrackingSpaceChoice { Rectangle, Trapezoid, Triangle, Cross, L_shape, T_shape, FilePath, Square, FourRectObstacle, OneRectObstacle };
    ...
    public enum PathSeedChoice { _90Turn, RandomTurn, StraightLine, Sawtooth, Circle, FigureEight, FilePath, RealUserPath , DenceWaypointRandomTurn};

    public static float obstacleParentHeight = 0.0018f;
    public static float bufferParentHeight = 0.0016f;

    [Header("MonteCarlo Simulation")]
    public bool montecarloSimulation = false;
    public MontecarloSimulation montecarloScript;

    public MontecarloSimulation.Distribution distribution = MontecarloSimulation.Distribution.Normal;
    public float meanError = 0.0f;
    public float stdError = 0.0f;
    ...
    public GameObject totalForceArrow;
    public GameObject futureArrow;
    public GameObject RedirectionTarget;
    public GameObject TrackingSpaceCenter;


    [Tooltip("angle threshold when Generate Virtual Path with Dence Waypoint Random Turn")]
    public float angleThreshold = 5.0f;

    [Tooltip("Length of generated Virtual Path using Path Seed : Dence Waypoint Random Turn")]
    public float pathLength = 100.0f;
    public float experimentTime = 180.0f;
    ...
    float timer = 0.0f;
    public bool getRandomMazeAllTrial = false;
    ...
    private void Awake()
    {
        ...
        var movementman = redirectedAvatars[0].GetComponent<MovementManager>();
        if(movementController == MovementController.AutoPilot)
        {
            if (getRandomMazeAllTrial)
            {
                try { 
        ...
        if (movementController == MovementController.AutoPilot)
        {
            ...
            {
                gameObject.AddComponent<MontecarloSimulation>();
                montecarloScript = gameObject.GetComponent<MontecarloSimulation>();
            }
            if (runInBackstage)
            {
        ...
        //pathSeedChoiceToWaypoints[PathSeedChoice.DenceWaypointRandomTurn] = VirtualPathGenerator.GenerateInitialPathByPathSeedHJ(PathSeed.GetPathSeedRandomTurn(), out sumOfDistances, out sumOfRotations, 3041 + experimentIterator);

        pathSeedChoiceToWaypoints[PathSeedChoice.DenceWaypointRandomTurn] = VirtualPathGenerator.GenerateInitialPathByPathSeedWithCollision(PathSeed.GetPathSeedDenceWaypointRandomTurn(angleThreshold, pathLength), pathLength, out sumOfDistances, out sumOfRotations, 3041 + experimentIterator, mapName);
    ...
    void FixedUpdate()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Q) || timer > experimentTime) {
            EndExperiment(1);
        }
    ...
    private void GenerateExperimentSetupsByUI()
    {
        ...
        //    mm.InitializeWaypointsPattern(); // Set movementManager.waypoints
        //    mm.initialConfiguration = initialConfigurations[i];
        //    var avatarInfo = mm.GetCurrentAvatarInfo(); // return movementManager.waypoints
        ...
        for (int i = 0; i < trialsForCurrentExperiment; i++)
        {
            ...
            for (int j = 0; j < redirectedAvatars.Count; j++)
            {
                var ra = redirectedAvatars[j];
                var mm = ra.GetComponent<MovementManager>();
                mm.InitializeWaypointsPattern(); // Set movementManager.waypoints
                mm.initialConfiguration = initialConfigurations[j];
                var avatarInfo = mm.GetCurrentAvatarInfo(); // return movementManager.waypoints
                avatarList.Add(avatarInfo);
            }
            ...
            if (getRandomMazeAllTrial)
            {
                ...
                mazeSpawner.generateMaze();
                mazeSpawner.increaseRandomSeed();
            ...
            var Ra = redirectedAvatars[0];
            float sumOfDistances, sumOfRotations;
            //pathSeedChoiceToWaypoints[PathSeedChoice.DenceWaypointRandomTurn] = VirtualPathGenerator.GenerateInitialPathByPathSeedHJ(PathSeed.GetPathSeedRandomTurn(),out sumOfDistances, out sumOfRotations, 3041 + experimentIterator);
            if (Ra.GetComponent<MovementManager>().pathSeedChoice == PathSeedChoice.DenceWaypointRandomTurn)
            {
                pathSeedChoiceToWaypoints[PathSeedChoice.DenceWaypointRandomTurn] = VirtualPathGenerator.GenerateInitialPathByPathSeedWithCollision(PathSeed.GetPathSeedDenceWaypointRandomTurn(angleThreshold, pathLength), pathLength, out sumOfDistances, out sumOfRotations, 3041 + experimentIterator, mapName);
                avatarList[0].waypoints = pathSeedChoiceToWaypoints[PathSeedChoice.DenceWaypointRandomTurn];
            }
    ...
    {        
        ...
        for (int id = 0; id < avatarList.Count; id++)
        {
            ...
            var mm = redirectedAvatars[id].GetComponent<MovementManager>();
            var rm = redirectedAvatars[id].GetComponent<RedirectionManager>();
            //if (avatarList[id].pathSeedChoice == PathSeedChoice.DenceWaypointRandomTurn)
            //{
            //    pathSeedChoiceToWaypoints[PathSeedChoice.DenceWaypointRandomTurn] = VirtualPathGenerator.GenerateInitialPathByPathSeed(PathSeed.GetPathSeedDenceWaypointRandomTurn(), 100, out sumOfDistances, out sumOfRotations, 3041 + experimentIterator);
            //    avatarList[id].waypoints = pathSeedChoiceToWaypoints[PathSeedChoice.DenceWaypointRandomTurn];
            //}

            //mm.InitializeWaypointsPattern(); // change mm.movementManeger.waypoints to pathSeedChoiceToWaypoints
            mm.LoadData(id, avatarList[id]); // waypoints = avatar.waypoints
            //reload data from experimentSetups

            mm.GenerateTrackingSpaceMesh(trackingSpacePoints, obstaclePolygons);
            
        }
        var movementManager = redirectedAvatars[0].GetComponent<MovementManager>();
        if(movementController == MovementController.AutoPilot)
        {
            if (montecarloSimulation)
            {
                montecarloScript.initializeMC();
            }
        ...
        if (getRandomMazeAllTrial)
        {
            ...
            if (experimentIterator == 0)
            {
                mazeSpawner.initRandomSeed();
                mazeSpawner.generateMaze();
            ...
            else
            {
                ...
                //mazeSpawner.setRandomSeed(3041 + experimentIterator);
                mazeSpawner.DestroyMaze();
                mazeSpawner.increaseRandomSeed();
                mazeSpawner.generateMaze();
            }
    ...
    void EndExperiment(int endState)
    {
        ...
        if (experimentIterator == experimentSetups.Count)
        {
            ...
            instanceHUD.GetComponent<TextMesh>().text = "Finish !!";
            instanceHUD.parent = rm.headTransform;
            instanceHUD.localPosition = instanceHUD.position;
            instanceHUD.localRotation = instanceHUD.rotation;
            if(rm.resetter.instanceHUD != null)
            {
                rm.resetter.DestroyHUD();
            }
    ...
    public void GeneratePhysicalWall(List<Vector2> trackingSpacePoints, List<List<Vector2>> obstaclePolygons)
    {
        GameObject parent = GameObject.Find("Plane");
        ...
        foreach (var polygon in polygons)
        {
            for (int i = 0; i < polygon.Count; i++)
            {
                ...
                Vector2 centerPoint = (p + q) / 2;
                GameObject wall = GameObject.Instantiate(physicalWall);
                wall.name = "physicalWall";
                wall.transform.position = new Vector3(centerPoint.x, 0.5f, centerPoint.y);

                //Debug.Log("centerPoint : " + centerPoint);
                wall.transform.localScale = new Vector3((q - p).magnitude,1,0.1f);
                var front = Utilities.RotateVector(q - p, 90).normalized;
                wall.transform.forward = new Vector3(front.x, 0, front.y);
                if (!showWall)
                {
                    wall.GetComponent<MeshRenderer>().enabled = false;
                }
                //Debug.Log("wall Forward : " + wall.transform.forward);
    ...
    public void GenerateTrackingSpace(int avatarNum, out List<Vector2> trackingSpacePoints, out List<List<Vector2>> obstaclePolygons, out List<InitialConfiguration> initialConfigurations)
    {
        ...
        switch (trackingSpaceChoice)
        {
            ...
            case TrackingSpaceChoice.Square://no obstacle in square
                ...
                break;
            case TrackingSpaceChoice.FourRectObstacle:
                TrackingSpaceGenerator.GenerateRectangleTrackingSpace(3, out trackingSpacePoints, out obstaclePolygons, out defaultInitialConfiguration, squareWidth, squareWidth);
                break;
            case TrackingSpaceChoice.OneRectObstacle:
                TrackingSpaceGenerator.GenerateRectangleTrackingSpace(4, out trackingSpacePoints, out obstaclePolygons, out defaultInitialConfiguration, squareWidth, squareWidth);
                //defaultInitialConfiguration = new List<InitialConfiguration>();
                //defaultInitialConfiguration.Add(new InitialConfiguration(new Vector2(0, squareWidth/2 - 2),Vector2.up));
                break;
            default:
                trackingSpacePoints = TrackingSpaceGenerator.GeneratePolygonTrackingSpacePoints(4);
                break;
        }
        GeneratePhysicalWall(trackingSpacePoints, obstaclePolygons);
