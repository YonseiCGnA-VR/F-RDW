...
public class ThomasAPF_Redirector : APF_Redirector
{
    ...
    public void ApplyRedirectionByNegativeGradient(Vector2 ng) {
        ...
        int desiredSteeringDirection = (-1) * (int)Mathf.Sign(Utilities.GetSignedAngle(redirectionManager.currDirReal, desiredFacingDirection)); // rotate world opposite to desiredFacingDirection?
        // desiredSteeringDirection --> if ng is right return -1
        ...
        // gain = 1 means that when User turn counter clockwise 90 degree, world that show to user in HMD turn clockwise 90 degree.

        var deltaDir = redirectionManager.deltaDir;
        Debug.Log("deltaDir : " + deltaDir);
        if (deltaDir * desiredSteeringDirection < 0) // Sign(deltaDir) == GetSignedAngle(currDir, desireDFacingDirection) --> rotate toward negative gradient 
        {
            // desiredSteeringDirection is Correct direction no problem
            ...
            Debug.Log("rotate toward : " + g_r);
        }
        else
        { // Sign(deltaDir) != GetSignedAngle(currDir, desireDFacingDirection) --> rotate away negative gradient
            // use max gain to turn more in physical world
