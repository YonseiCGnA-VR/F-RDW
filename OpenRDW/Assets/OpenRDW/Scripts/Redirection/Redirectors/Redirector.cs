...
public abstract class Redirector : MonoBehaviour
{
    ...
    /// <summary>
    /// Applies rotation to Redirected User. The neat thing about calling it this way is that we can keep track of gains applied. Positive means rotate ClockWise
    /// </summary>
    ...
    protected void InjectRotation(float rotationInDegrees)
    {
        ...
        {
            transform.RotateAround(Utilities.FlattenedPos3D(redirectionManager.headTransform.position), Vector3.up, rotationInDegrees); // Axis is "Up Vector" so it is opposite that we are seeing
    ...
    {
        //Debug.Log("rotationInDegrees " + rotationInDegrees);
        //Debug.Log("Curvature rotationInDegrees : " + rotationInDegrees);
        if (rotationInDegrees != 0)
        {                                
